...
import styles from "./edit-wishlist-modal.module.css"

interface EditWishlistFormData {
  name: string
  description: string
  isPublic: boolean
  coverImage: string
...
interface EditWishlistModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (wishlistData: EditWishlistFormData) => void
  initialData: {
    name: string
    description: string
    isPublic: boolean
...
export default function EditWishlistModal({ isOpen, onClose, onSubmit, initialData }: EditWishlistModalProps) {
  const [formData, setFormData] = useState<EditWishlistFormData>({
  ...
  const [errors, setErrors] = useState<Partial<EditWishlistFormData>>({})
  const [isSubmitting, setIsSubmitting] = useState(false)

  const validateForm = (): boolean => {
    const newErrors: Partial<EditWishlistFormData> = {}
  ...
  const handleInputChange = (field: keyof EditWishlistFormData, value: string | boolean) => {
